...
export default function FeelingFriends({ childName }) {
  ...
  const [currentIndex, setCurrentIndex] =
    useState(0); /*State to track current index */
  const [feedback, setFeedback] =
    useState(""); /*state to set feedback after child selects option */
  ...
  const [questions, setQuestions] = useState(() =>
    shuffleArray(feelingFriendsQuestions),
  );
  ...
  if (currentIndex >= questions.length) {
    /*logic to show end of game feedback */
    return (
      <div className="end-game-container">
        <h1 className="end-game-feedback">
          {childName
            ? `Congratulations ${childName}! You finished the game!`
            : `Congratulations! You finished the game!`}
          {/*ternary operator...shows childName if there is one...if not displays general message */}
        </h1>
        <p className="score-feedback">
          You got {score} out of {questions.length} questions right! Great job!
        </p>
        <button
  ...
  function nextQuestion() {
    /*goes to next creature index, resets feedback and image loaded status */
  ...
  const options = currentQuestion.options.map(
    (
      option /*maps through the options in my array to display all options */,
    ) => (
      <div className="option-button-container" key={option}>
        ...
        >
          {option} {feelingEmojis[option].emoji}
          {/*shows the option and the corresponding feeling emoji*/}
        </button>
      </div>
    ),
  );
  ...
  return (
    <div className="feeling-friends-container">
      ...
      {/*uses string interpolation to pull the current name and creature into the question */}
      ...
      <img
        ...
        onLoad={() =>
          setImageLoaded(true)
        } /* event handler that fires once the image has fully downloaded and rendered */
        style={{
          display: imageLoaded ? "block" : "none",
        }} /* hides the image while it's loading, shows it when it's ready. */
      />
